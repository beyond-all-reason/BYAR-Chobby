...
local function GetLobbyTabControls()
	...
	children[#children + 1] = Label:New {
		...
		tooltip = "Old Server is road-flag.bnr.la, new is bar.teifion.co.uk. Changing this will log you out of current server, click Login in top right to reconnect to new one, Temporarily we are on : server1.beyondallreason.info",
	}

	local barservers = {"server1.beyondallreason.info","road-flag.bnr.la"}
...
local function GetVoidTabControls()
	...
	children[#children + 1] = EditBox:New {
		...
		--font = Configuration:GetFont(2),
		useIME = false,
		tooltip = "Requires a lobby restart for changes to take effect. Old Server is road-flag.bnr.la, new will be bar.teifion.co.uk. Temporarily we are on server1.beyondallreason.info",
		OnFocusUpdate = {
			function (obj)
				if obj.focused then
					return
